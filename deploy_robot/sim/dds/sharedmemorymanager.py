...
class SharedMemoryManager:
        ...
        try:
            with self.lock:
                ...
                if len(json_bytes) > self.size - 8:  # reserve 8 bytes for length and timestamp
                    logger_mp.warning(
                        "SharedMemory '%s' data too large (%d > %d)",
                        self.shm_name, len(json_bytes), self.size - 8
                    )
                    return False
            ...
            logger_mp.exception("Error writing to shared memory '%s'", getattr(self, "shm_name", "?"))
            return False
    ...
    def read_data(self) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            logger_mp.exception("Error reading from shared memory '%s'", getattr(self, "shm_name", "?"))
            return None
        ...
        if hasattr(self, 'shm') and self.shm:
            ...
            if self.created:
                try:
                    ...
                    # Best-effort; another process may have unlinked
                    pass
