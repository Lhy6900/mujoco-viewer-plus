        ...
        if not self.cfg.headless:
            if getattr(self.cfg, "viewer_type", "native") == "viewer_plus":
                try:
                    ...
                    self.viewer = ViewerPlus(self.model, self.datalist[self.cfg.num_envs - 1], config=vcfg, num_envs=self.cfg.num_envs)
                except Exception:
                    # Fallback to native viewer on any error
                    logger_mp.exception("Failed to initialize ViewerPlus, falling back to native viewer")
                    self.viewer = mujoco.viewer.launch_passive(self.model, self.datalist[self.cfg.num_envs - 1])
            else:
                self.viewer = mujoco.viewer.launch_passive(self.model, self.datalist[self.cfg.num_envs - 1])
