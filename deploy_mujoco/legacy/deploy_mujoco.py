...
if __name__ == "__main__":
    ...
    # 设置摄像头参数（可能没有设置相机）
    # m.cam_pos[0] = [0,-2,0.78]
    # m.cam_quat[0] = m.body_quat[1]
    # m.cam_pos[1] = [0,2,0.78]
    # m.cam_quat[1] = m.body_quat[1]
    # m.cam_pos[2] = [2,0,0.78]
    # m.cam_quat[2] = m.body_quat[1]
    # m.cam_pos[3] = [-2,0,0.78]
    # m.cam_quat[3] = m.body_quat[1]
        ...
        while viewer.is_running() and time.time() - start < simulation_duration and running:
            ...
            if apply_external_force and force_body_id != -1:
                ...
                if relative_time >= 0 and relative_time < force_duration:
                    segment_duration = force_duration / 3.0
                    current_force_mag = 0.0

                    if relative_time < segment_duration: # 增长阶段
                        current_force_mag = force_magnitude * (relative_time / segment_duration)
                    elif relative_time < 2 * segment_duration: # 保持阶段
                        current_force_mag = force_magnitude
                    else: # 减小阶段
                        current_force_mag = force_magnitude * (1 - (relative_time - 2 * segment_duration) / segment_duration)
                    
                    # 计算外力向量并施加
                    applied_force_vector = current_force_mag * force_direction_world
                    d.xfrc_applied[force_body_id, :3] = applied_force_vector
                    # print(f"时间: {current_time:.3f}s, 施加力: {current_force_mag:.2f}N, 向量: {applied_force_vector}")
                else:
                    # 在非施力时间段，确保外力为零
            ...
            if counter % control_decimation == 0:  # 每隔control_decimation步更新一次
                ...
                # 检查是否到达轨迹末尾
                if traj_counter >= traj_total_length - 1:
